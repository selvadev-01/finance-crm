...
function server(
  ...answers: (
    number | "network" | ((body: { idempotencyKey: string }) => Response)
  )[]
) {
  const sent: {
  ...
  const fetch = (async (_url: string, init?: RequestInit) => {
    ...
    const payload =
      answer < 300
        ...
        : {
            code: answer === 401 ? "UNAUTHENTICATED" : "ACCOUNT_NOT_ACTIVE",
            message: "refused",
          };
...
describe("offline outbox (offline-sync.md, BR-13)", () => {
  ...
  describe("US-052 sync", () => {
    it("drains oldest first, and one refusal does not block the others", async () => {
      ...
      expect(sent.map((body) => body.accountLoanId)).toEqual([
        "acc-1",
        "acc-2",
        "acc-1",
      ]);
    ...
    it("an account waiting to retry holds back its later collections, so they apply in the order taken; other accounts carry on", async () => {
      ...
      const byAccount = (await listOutbox(db)).map((entry) => [
        entry.payload.accountLoanId,
        entry.status,
      ]);
    ...
    it("a request that hangs counts as a network failure after the timeout, so the drain finishes and the lock is released", async () => {
      ...
      const report = await drainOutbox(db, {
        fetch: hanging,
        locks: null,
        requestTimeoutMs: 50,
      });
    ...
    it("retry one makes a backed-off entry due now; a refused entry cannot be retried; the last sync time is kept", async () => {
      ...
      const refused = await record(
        ...
        "acc-2",
        "80",
        new Date("2026-09-14T04:05:00Z"),
      );
      const at = Date.parse("2026-09-14T05:00:00Z");
      await drainOutbox(db, {
        fetch: server("network", 422).fetch,
        locks: null,
        now: () => at,
      });
      ...
      expect(again.sent.map((body) => body.idempotencyKey)).toEqual([
    ...
    it("a collection saved while a drain is sending is sent by that drain before it lets go of the lock", async () => {
      ...
      const fetch = (async (_url: string, init?: RequestInit) => {
        ...
        if (!second)
          second = (
            await record(db, "acc-2", "150", new Date("2026-09-14T04:00:05Z"))
          ).idempotencyKey;
        return new Response(
          JSON.stringify({ ...body, id: `col-${sent.length}`, account: {} }),
          { status: 201 },
        );
      }) as typeof globalThis.fetch;
      ...
      expect((await listOutbox(db)).map((entry) => entry.status)).toEqual([
        "SYNCED",
        "SYNCED",
      ]);
    });
  ...
  describe("US-060 the phone tells the office what it still holds", () => {
    it("reports the unsent count and the oldest capture time, and nothing once everything is sent", async () => {
      ...
      expect(await queueReport(db)).toEqual({
        unsentCount: 2,
        oldestUnsentAt: "2026-09-14T04:00:00.000Z",
      });
      ...
      expect(await queueReport(db)).toEqual({
        unsentCount: 1,
        oldestUnsentAt: "2026-09-14T05:00:00.000Z",
      });
    });
  });
  ...
  describe("queue limits", () => {
    it(`warns at ${WARN_AT} unsynced and refuses new entries at ${BLOCK_AT}`, async () => {
      ...
      expect(await summarise(db)).toMatchObject({
        unsynced: WARN_AT,
        warn: true,
        blocked: false,
      });
      for (let i = WARN_AT; i < BLOCK_AT; i += 1)
        await record(db, "acc-1", "1");
      await expect(record(db, "acc-1", "1")).rejects.toBeInstanceOf(
        OutboxFullError,
      );
  ...
  describe("US-051 route available offline, balances updated locally", () => {
    it("applies unsynced collections to the cached route, marked optimistic; a refused one is not applied", async () => {
      ...
      expect(first).toMatchObject({
        ...
        collectedToday: { amount: "100", classification: "CORRECT" },
      });
    ...
    it("a refresh that lands while the entry is still syncing, after the server committed it, does not reduce the balance twice", async () => {
      ...
      await drainOutbox(db, {
        fetch: committedButUnanswered.fetch,
        locks: null,
      });
    ...
    it("a refused collection can be removed once acknowledged; nothing still on its way can be", async () => {
      ...
      await drainOutbox(db, {
        fetch: server(422, "network").fetch,
        locks: null,
      });
      expect(await canSignOut(db)).toBe(false);
      ...
      expect(
        (await listOutbox(db)).map((entry) => entry.idempotencyKey),
      ).toEqual([waiting.idempotencyKey]);
    });
    ...
    it("signing out clears the route, identity and sent entries — and clears nothing while a collection is unsent", async () => {
      ...
      expect([
        await db.count("route"),
        await db.count("meta"),
        await db.count("outbox"),
      ]).toEqual([0, 0, 0]);
    });

    it("gives each row its S-01 state: pending, saved on phone, syncing, synced — and a refused entry leaves it pending", async () => {
      const served = route();
      served.customers[0]!.accounts.push({
        ...served.customers[0]!.accounts[0]!,
        accountLoanId: "acc-3",
        accountCode: "ACC-3",
      });
      served.customers[0]!.accounts[1]!.collectedToday = {
        amount: "80.00",
      ...
      expect((await readRoute(db, TODAY))!.rowState).toEqual({
        "acc-1": "SAVED",
        "acc-2": "SYNCED",
        "acc-3": "PENDING",
      });
    ...
    it("is stale after 72 hours", async () => {
      await storeRoute(db, route(), new Date("2026-09-14T03:00:00Z"));
      expect(
        (await readRoute(db, TODAY, new Date("2026-09-17T02:59:00Z")))!.stale,
      ).toBe(false);
      expect(
        (await readRoute(db, TODAY, new Date("2026-09-17T03:01:00Z")))!.stale,
      ).toBe(true);
    });
