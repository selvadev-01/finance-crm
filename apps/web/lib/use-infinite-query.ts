...
export type InfiniteState<Route extends RouteDefinition> = QueryState<Route> & {
  reload: () => void;
...
/**
 * A cursor list that grows as it is scrolled (api-design.md#pagination): the
 * first page through `useApiQuery`, then "Show more" appends pages. Changing
 * the request — a filter — starts again from the first page. Nothing is sorted
 * or filtered here; the API's order stands.
 *
 * For the **notification panel and the Junior's field app only**, where rows
 * live in a popover or a single phone column with no room for a pager. Console
 * lists use `usePagedQuery`, which walks the same cursors a page at a time and
 * says where the reader is.
 */
export function useInfiniteQuery<Route extends RouteDefinition>(
  route: Route,
