...
- `packages/db` — Prisma 7, the full 28-table schema, forty-six applied migrations; every data-dictionary invariant is a database constraint (CHECKs, triggers, partial uniques)
...
**The offline E2E suite writes permanent rows — the one approved exception to the rules below.** `pnpm --filter offline-e2e test:offline` (needs `pnpm build` for the API first; it builds and starts the web app itself, in Chrome) creates a new "Offline E2E &lt;run id&gt;" organisation each run with disbursed accounts, collections and ledger postings that can never be deleted. The user accepted this on 2026-09-14 so the offline path is proven against the real API. Run it deliberately, not as a reflex. Because of it, **no test may assume an append-only table is globally empty** — count only the rows the test created.

**`pnpm --filter offline-e2e test:regression` is the only other suite under that exception**, extended by the user on 2026-09-22. It is the Super Admin → Admin → Senior → Junior regression journey in `apps/offline-e2e/regression/`: one business day through the real screens and API, with Chennai data (`regression/data.ts`) — only the organisation and its Super Admin are written directly, and that owner appoints the Admin, who creates the Senior and Junior, sectors, area lines, assignments, customers and accounts (disbursed, pending and mid-term); the Senior sets the visiting order; the Junior collects on `/route`, asks for a correction and hands over the cash; the Senior approves, acknowledges and closes the day; then the books are read back from the database. It leaves a "Sri Kamakshi Finance — Regression &lt;run id&gt;" organisation per run. It stays online, so it reuses a running `pnpm dev`; otherwise it starts the built API and a production web build. It skips on a Sunday, a holiday, and within 45 minutes of midnight IST, where the business date would change mid-run. **Do not extend the exception to a third suite.**
