...
The root layout sets `compact` because the console is the larger surface. The Junior's route segment sets `comfortable` on its own layout, and every control inside widens with no component change. The console's **phone layout** (`MobileShell`, ADR-0016) is `comfortable` too, and it also raises `--control-height-sm` to the 44px touch height, so the pager's Previous and Next and a row's small action are thumb-sized there.
...
| `Tabs.Root/List/Trigger/Content`                 | The parts of one record (S-09). Control `value` from the screen so it can live in `?tab=`. Not for page navigation — a group of **pages** switched between by link wears the same strip through `tabListClass` and `tabLinkClass` (`apps/web/components/tab-links.tsx`, used by Settings), which mark the current one with `aria-current="page"` rather than a `data-state`. Both read from one definition, so a link tab and a real tab cannot drift apart |
...
| `MobileShell.Root/AppBar/Main/TabBar/Tab`                                 | The console's phone layout (ADR-0016): a 56px app bar clear of the notch, one column, and a 64px bottom tab bar above the home indicator. `Tab` takes a link or button as its child, `state="current"` or `"idle"` (current is a teal-wash pill), and an optional `count`                                                                                                                                                             |
| `arrowLinkClass`                                                          | A standalone "Open day close →" link at the foot of a card or section: accent text with a 44px tap target, its padding cancelled by a negative margin so the layout does not move                                                                                                                                                                                                                                                     |
| `Dialog placement="sheet"`                                                | A dialog that rises from the foot at full width, for the phone layout's "More" and choices. `placement="center"` (the default) is the floating dialog                                                                                                                                                                                                                                                                                 |
...
| `StatGrid` + `Stat`                                                       | Figures in one ruled frame (`columns` 2, 3 or 4). Below 640px every grid is two to a row, an odd last figure spans the row, and values drop one size (`heading`, or `title` in tiles) so a crore amount fits. `Stat tone` colours the figure only when it is itself a status (a shortfall). The caller formats money. `frame="tiles"` makes each figure a flat tile with an `icon` (`iconTone`) and an optional `delta` (`DeltaChip`) |
...
| `AreaChart`                                                               | A money series over days, hand-drawn SVG. Values are decimal strings and become plot fractions only in `chart-scale.ts`, the one place in the UI a money string meets a number. It is an image with a summary, a hidden table of every point, and ←/→ to read one day at a time. Below 640px every other day label is dropped (never the first or last)                                                                               |
...
| Component                   | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `DataView`                  | TanStack Table 8, shown as a table from 768px and as cards below. Columns are TanStack `ColumnDef`s with `meta.align`, `meta.hideOnCard` and `meta.card`. On the phone card, `card: "headline"` puts that cell (the row's key amount) large beside the name and `card: "status"` puts the badge under it; every other field is a small label over its value, two to a row. The column builders take `card` too. **Sorting is offered only when `complete`**: sorting the first page of a longer list would mislead |
| `DataView footer`           | A `ListPager` (or, where a list scrolls, a `ListFooter`) passed as `footer` becomes the bottom band inside the table frame (below the cards on a phone), as in the Stitch designs                                                                                                                                                                                                                                                                                                                                  |
| `CodeChip`                  | A short code in a quiet mono chip beside a name: `LN-07` Market Road                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `FilterBar` + `FilterField` | The controls above a list, in a light framed panel: labelled, `width` `sm`, `md` or `lg`. Below 768px the filters fold behind a "Filters" button, so the first records are on a phone's first screen; pass `summary` ("14 Sep 2026 to 20 Sep 2026, all lines") so the folded bar still says what the list shows                                                                                                                                                                                                    |
| `ListPager`                 | The foot of a console list (ADR-0017): "51–100 of 1,234 customers", a Rows choice of 10, 25, 50 or 100, and First / Previous / Next with "Page 2 of 25" between them. **There is no last page and no jumping to a page number** — a page is walked to over the API's cursor. The range is a `role="status"` live region, so stepping is announced and not only drawn. Wire it with `apps/web/components/pager.tsx` over `usePagedQuery`                                                                            |
| `ListFooter`                | "N items" and "Show more", which **loads by itself as the reader scrolls near the end** (its `LoadMoreSentinel`, 2026-09-22). Now only where a pager has no room: the notification panel and the field app's own lists. A failed page waits 5 seconds before the marker may ask again (`useInfiniteQuery`)                                                                                                                                                                                                         |
| `LoadMoreSentinel`          | An invisible marker after a long list's last row that calls `onMore` once it is within 600px of the viewport, and not while `busy`. **Long lists load a piece at a time in the one page scroll, never in a scroll box of their own.** The field app draws its on-phone lists (route, Customers, today's Collections, Sync's sent entries) 20 at a time with `useLazyCount` (`app/route/lazy-list.tsx`); what is still unsent on the phone is always shown in full                                                  |
