...
The root layout sets `compact` because the console is the larger surface. The Junior's route segment sets `comfortable` on its own layout, and every control inside widens with no component change. The console's **phone layout** (`MobileShell`, ADR-0016) is `comfortable` too, and it also raises `--control-height-sm` to the 44px touch height, so "Show more" and a row's small action are thumb-sized there.
...
| `ListFooter`                | "N items" plus the next cursor page, which **loads by itself as the reader scrolls near the end** (its `LoadMoreSentinel`, 2026-09-22); "Show more" stays for the keyboard. A failed page waits 5 seconds before the marker may ask again (`usePagedQuery`)                                                                                                                                                                                                                                                        |
