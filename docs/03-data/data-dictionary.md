...
| `routePosition`   | `Int`            | Yes  | Place on the line's visiting order (US-040), from 1; set by the line's Senior or an Admin. Null: not yet placed — after the placed ones, in code order. Cleared on a line transfer. CHECK `customer_route_position_check` (`>= 1`); partial unique `customer_line_route_position_key` on `(lineId, routePosition)` where not null |
...
| `entityId`       | `String`      | No   |                                                                                                                                                                                                                                                  |
| `action`         | `AuditAction` | No   | `CREATE` \| `UPDATE` \| `DELETE` \| `APPROVE` \| `REJECT` \| `LOGIN` \| `REOPEN_DAY` \| `EXPORT` (a download, M12: `entityTable` is `export`, `entityId` the export's name)                                                                      |
| `before`         | `Json`        | Yes  |                                                                                                                                                                                                                                                  |
| `after`          | `Json`        | Yes  |                                                                                                                                                                                                                                                  |
| `ipAddress`      | `String`      | Yes  |                                                                                                                                                                                                                                                  |
| `userAgent`      | `String`      | Yes  |                                                                                                                                                                                                                                                  |

Constraint (`constraints_audit_log_organization`, `NOT VALID` so it applies to new rows only): `organizationId` is set unless `action = LOGIN`. Constraint `audit_log_export_check` (`constraints_audit_export`): `action = EXPORT` exactly when `entityTable = 'export'`, and an export has an actor and an `after`. Indexed on `(organizationId, createdAt)` for the log's newest-first reads.
