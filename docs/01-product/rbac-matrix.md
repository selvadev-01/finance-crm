...
> **Scoping applies to historical rows by the row's own attribution, not the viewer's current line.** A Senior moved from Line 3 to Line 7 sees Line 7's data — including collections recorded on Line 7 before they arrived, and *not* including the Line 3 history they used to supervise. The line is the unit of responsibility; the person is not.
>
...
> **Failure semantics:** a scoped-out row returns `404`, not `403`. A Junior probing another line's customer IDs learns nothing about whether they exist. Denied *actions* on visible rows return `403`, which is safe because the row's existence is already known.
