...
- **Tier 2 — HTTP tests** write for real. Every identifying value they create carries the run's `testRunTag` (use `testEmail()`), and `deleteTestRunData` removes exactly those rows. A Tier 2 test that creates domain rows must extend that function with a tagged delete per table — never a bulk delete.
