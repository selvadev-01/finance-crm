...
## Phase 0 — Foundations · ~1 week · **in progress**

**Done.** Turborepo + pnpm workspace. `apps/web` (Next.js 16, React 19) and `apps/api` (NestJS 12, ESM, Vitest) both boot. `@repo/ui`, `@repo/eslint-config`, `@repo/typescript-config`. Root `dev`, `build`, `lint`, `check-types`, `test` and `format` scripts.

**Remaining.** Local PostgreSQL with `rasi_dev` and `rasi_test`. `packages/db` — Prisma schema, first migration, seed. `packages/domain` and `packages/contracts` created as empty, lint-clean packages, so the boundaries exist before there is code to misplace. Better Auth generated and mounted ([authentication](../02-architecture/authentication.md)). Test harness against `rasi_test` with transaction-rollback isolation.

**Exit:** `pnpm dev` runs everything; a user can sign in; `pnpm test` passes against `rasi_test`.
